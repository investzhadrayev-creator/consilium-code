# __build__ = "v4.2.81"   # ЕДИНАЯ версия СБОРКИ микросервиса: deploy пушит microservice/
...
def peer_pe_inhouse(peers, tiingo_token, errors):
    ...
    for p in (peers or [])[:6]:
        ...
        try:
            ...
            # v4.2.79: the brief's peer table needs size, not just the multiple — a 23x median over
            # three names is read differently once the reader can see whether those names are the
            # company's size or a tenth of it. Both come from data already fetched above: shares
            # from the same EDGAR pull, revenue from the same filing. No new collection.
            if sh and sh[-1].get("val"):
                row["shares_diluted"] = sh[-1]["val"]
                if price:
                    row["market_cap"] = round(sh[-1]["val"] * price, 0)
            rev = (ef.get("revenue") or [])
            if rev and isinstance(rev[-1].get("val"), (int, float)):
                row["revenue_ltm"] = rev[-1]["val"]
                row["revenue_fy_end"] = rev[-1].get("end")
        except Exception as e:
            row["error"] = str(e)[:100]
        rows.append(row)

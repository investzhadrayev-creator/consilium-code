...
(async () => {
  ...
  // v4.2.76 — LEAVES, not containers. The ORCL diagnostic reported "4 live of 19"; three of the
  // four were headings whose own paragraph held no marker while both sub-sections beneath them
  // were dead. A container inherits nothing from its children and so looked alive by construction
  // — and the 6/25 baseline it was compared against was inflated the same way, which is precisely
  // why the comparison concluded "no change": both sides were wrong in the same direction.
  await check('v4.2.76: a heading over dead sub-sections is NOT counted as live', async () => {
    // Local, not the shared `r`: reassigning it would hand this fixture to the pin that runs next.
    const rLeaf = await run(FP('\n## 1. Revenue\n\nintro paragraph with no marker\n'
      + '\n### 1a. Annual\n\nno data [UNVERIFIED]\n'
      + '\n### 1b. Quarterly\n\nno data [UNVERIFIED]\n'
      + '\n## 2. News\n\nreal fact with a date\n'), GT);
    const v = rLeaf.out._fp_vectors;
    assert(v.total === 3, 'the container was counted as a vector: total=' + v.total);
    assert(v.unverified === 2, 'unverified=' + v.unverified);
  });
  ...
  await check('both assemblers READ the count and neither computes it', () => {
    ...
    for (const a of asm) {
      ...
      // v4.2.73: the brief no longer routes the count through an `fpLine` variable — it prints the
      // two kinds of data separately, in prose. What the pin guards is that the count REACHES the
      // page, not the name of the variable carrying it; a pin on the variable name was a proxy,
      // and the proxy would have blocked a correct rewrite.
      assert(/fpLine/.test(code) || /Новостной контекст/.test(code) || /fpv\.total/.test(code),
        a.name + ' reads the count but never prints it');

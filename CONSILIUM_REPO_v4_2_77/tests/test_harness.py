...
class TestDataIntegrityGates(unittest.TestCase):
    ...
    def test_debt_uncertain_forbids_full_marks_on_leverage(self):
        ...
        d_clean = gps_quant(clean)["detail"]["D"]["pts"]["de"]
        d_disp = gps_quant(disputed)["detail"]["D"]["pts"]["de"]
        self.assertGreater(d_clean, d_disp,
                           "a disputed debt reading must score BELOW a clean one on leverage")
        self.assertTrue(gps_quant(disputed)["detail"]["D"]["debt_uncertain"])

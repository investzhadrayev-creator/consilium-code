...
const RESP = {
  'Stage 1 Perplexity Raw': { usage: { prompt_tokens: 4000, completion_tokens: 2000 } },
...
check('D.2: an on-path stage whose reference throws is a LOST METER, not "not run"', () => {
  ...
  const $throwStage1 = (name) => {
    if (name === 'Stage 1 Perplexity Raw') throw new Error('custody: long-range reference died');
  ...
  const s1 = out.stages.find(x => x.stage === 'Stage 1 Perplexity Raw');
...
const GOOD = {
  ...
  totals: { input_tokens: 69000, output_tokens: 13000, est_cost_usd: 0.55,
            ...
            excluded_meter_lost: [], understated_incomplete: ['Stage 1 Perplexity Raw'] },

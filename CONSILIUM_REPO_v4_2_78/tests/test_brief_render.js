...
(async () => {
  ...
  await check('v4.2.77 (g).3: only DATED lines from sections 5 and 12 become catalysts',
    async () => {
      ...
      // v4.2.78: the events arrive in English inside a Russian document, and that is deliberate —
      // translating them would run facts through model judgment a second time, and dates and
      // figures are what such a pass loses first. Unexplained, the reader reads it as a defect.
      assert(/на языке источника/i.test(t) || /НА ЯЗЫКЕ ИСТОЧНИКА/.test(t),

...
check('the graph is wired: four calls in, one edge to the entity gate', () => {
  const c = WFJ.connections;
  for (const t of ['fin', 'legal', 'compete', 'news']) {
    const tgt = [].concat.apply([], (c['Stage 1 FP ' + t] || {}).main || []).map(x => x.node);
    assert(tgt.indexOf('Merge FACT_PACK Calls') !== -1, 'call not wired to the merge: ' + t);
  }

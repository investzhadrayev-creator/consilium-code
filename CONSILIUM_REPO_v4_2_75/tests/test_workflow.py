...
class TestDisabledStageIsDocumented(unittest.TestCase):
    """v4.2.70 (mandate a) — a stage removed from the graph must say so where it is consumed.

    The Grok stage has had no edge in or out since before v4.2.44. Its absence was reconstructed by
    archaeology across eleven versions — and the reason was there all along, written into the very
    prompt that lost the input, with the date, the cost audit that justified it, and the exact steps
    to re-enable. Three places were searched (ledger, connections, node flags) and not the fourth.
    This pin makes the declaration load-bearing so it cannot be deleted quietly along with the code
    it explains.
    """

    def test_the_consumer_declares_the_missing_input(self):
        body = json.dumps(NODES["Stage 2b Claude"]["parameters"], ensure_ascii=False)
        self.assertIn("Stage 3 crowd sentiment DISABLED", body,
                      "the consumer of the removed stage no longer explains its absence")
        for token in ("operator decision", "2026-07-18", "Re-enable"):
            self.assertIn(token, body,
                          "the declaration lost its %s — a rationale without one is folklore"
                          % token)

    def test_the_stage_is_absent_from_the_graph_in_both_directions(self):
        conns = json.dumps(WF.get("connections", {}), ensure_ascii=False)
        self.assertNotIn('"Stage 3 Grok"', conns,
                         "the stage was re-wired without updating the declaration in Stage 2b")
...
class TestResilience(unittest.TestCase):
    ...
    LLM_NODES = ["Stage 1 FP fin", "Stage 1 FP legal", "Stage 1 FP compete", "Stage 1 FP news", "Stage 2a Claude", "Stage 2b Claude", "Stage 3 Grok",
...
class TestNoSecretsInWorkflow(unittest.TestCase):
    ...
    def test_llm_nodes_use_credentials_not_inline_headers(self):
        for name in ["Stage 1 FP fin", "Stage 1 FP legal", "Stage 1 FP compete", "Stage 1 FP news", "Stage 2a Claude", "Stage 2b Claude", "Stage 3 Grok",
...
class TestMeterMapTopologyV4221(unittest.TestCase):
    ...
    def test_core_p_llm_stages_are_NOT_in_the_map(self):
        ...
        code_map = self._map_from_code()
        for on_path in ("Stage 1 FP fin", "Stage 1 FP legal", "Stage 1 FP compete", "Stage 1 FP news", "Stage 2a Claude", "Stage 2b Claude",
                        "Stage 4 Gemini", "Stage 5 Auditor", "Stage 6 Arbiter"):

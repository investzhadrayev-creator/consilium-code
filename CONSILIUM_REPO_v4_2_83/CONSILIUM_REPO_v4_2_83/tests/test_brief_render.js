...
const RESULT = {
  ...
  gps: { total: 70, max: 100, blocks: [
    ...
    // v4.2.83: the C and B blocks were ABSENT from this fixture, so the two lines that call the
    // ratio helper never rendered and `d9-01` — the pin guarding one multiplier format — could not
    // fail. That is the pin the `28`/`27.53` defect walked past. A fixture missing the block is a
    // fixture missing the branch: CLAUDE.md already says adding a render section means extending
    // the mock in the same change, and the reverse holds too.
    { name: 'B (profitability)', points: 5, max: 15,
      evidence: { op_margin_series: [0.3417, 0.3059], fcf_conversion: -1.3862 } },
    { name: 'C (valuation)', points: 10, max: 15,
      evidence: { peg: 0.734, fwd_pe_vs_sector: 0.6566 } } ] },
  ...
  // v4.2.83 (мандат 07 §4): the fixture carries the THREE-state split, because the brief now
  // publishes it. A two-state stub here would have let the surface go on printing "собран 1 из 9"
  // while the pin stayed green — the shape that let this defect reach a paid run.
  _fp_vectors: { total: 9, unverified: 8, empty: 1, caveated: 7, live: 1, pct: 0.889,
                 data_questionable: true, threshold: 0.30,
                 states_note: 'EMPTY = the section says it found nothing; CAVEATED = content plus '
                   + 'an [UNVERIFIED] note; LIVE = content, no caveat. `unverified` = empty + '
                   + 'caveated, kept for continuity with runs before v4.2.82 — those two numbers '
                   + 'are NOT comparable across the contract change.' },
  ...
  three_year_table: { price_used: 141.85, pe_basis: "today's price / that year's EPS",
    rows: [{ fy_end: '2024-05-31', revenue: 5.28e10, net_income: 1.05e10, eps: 3.77,
           ...
           // v4.2.83: a NEGATIVE figure, because ORCL's FY2025 free cash flow really is one and
           // because `d2-02` (the minus must not be swallowed by the currency mark) could not
           // fail against a table of positives. The helper under test has a sign branch; a
           // fixture without a negative never reaches it.
           { fy_end: '2025-05-31', revenue: 5.72e10, net_income: 1.22e10, eps: 4.34,
             fcf: -3.94e8, pe_at_todays_price: 32.68 },
...
(async () => {
  ...
  await check('news section: HONEST ABSENCE when no news vectors survived', async () => {
    ...
    // v4.2.83: this pin used to assert the EXACT sentence "Новостные данные этого прогона
    // неполны". Mandate 07 §4 replaced that sentence because it named three sections from a
    // constant, and the pin went red over a wording change with no defect behind it — rule 6, a
    // pin that checks the form forbids a change rather than a defect. It now checks the property:
    // an absence is STATED, and it is stated as ours rather than as a fact about the company.
    assert(/годных для показа здесь, этот прогон не дал|Новостные данные этого прогона неполны/.test(md),
      'silent absence instead of an honest line');
    assert(/Числа из отчётности SEC этим не затронуты/.test(md),
      'the reader is not told which half the gap does not touch');
  ...
  await check('v4.2.73: the two kinds of data are marked SEPARATELY', async () => {
    ...
    // v4.2.83: was pinned to "собрано 0 разделов из 14", the two-state wording. The property it
    // guards is that the news half is COUNTED and counted apart from the verified half — not the
    // sentence that carried it.
    assert(/Новостной контекст: собрано \d+ раздел\S*/.test(md),
      'the news half must be counted, separately from the verified half');
    assert(/с оговоркой/.test(md),
      'the caveated state must be visible, else a section with content reads as nothing');
  ...
  await check('v4.2.69: only NON-ZERO factors are named in the gap prose', async () => {
    ...
    // v4.2.83: every factor in this fixture is non-zero, so removing the "only if non-zero" guard
    // changed nothing and `prose-01` could not fail. The property is that a factor which did NOT
    // act is not listed among the ones that did — so it needs a render where growth is the zero.
    const md3 = await renderMd({ central_lens: Object.assign({}, RESULT.central_lens, {
      iv: 120.00, delta_iv_vs_verdict_pct: -5.1,
      gap_decomposition: { verdict_iv: 126.49, base_change: -6.5, growth_change: 0,
        multiple_change: 0, bases_differ: true, multiple_differs: false,
        note: 'they do not sum' } }) });
    const factors = md3.slice(md3.indexOf('Что именно её двигает'),
                              md3.indexOf('Величины не складываются'));
    assert(factors.indexOf('темп роста') === -1,
      'a factor with ZERO contribution was listed among the ones that moved the valuation');
    assert(/база расчёта/.test(factors), 'the factor that DID act was not named');
  });
  ...
  await check('v4.2.79 (I): three fiscal years, each row carrying its fiscal-year END', async () => {
    ...
    // v4.2.83: the pin checked the row DATES and not the column HEADER, so renaming the column to
    // «год» left it green — and «год» is exactly the ambiguity the section exists to remove.
    assert(/фискальный год \(конец\)/.test(t),
      'the column header no longer says the dates are fiscal-year ENDS');
  ...
  await check('D4b: a bare date is not an event either', async () => {
    // v4.2.83 (mandate 09 §1). `cata-01` mutates away the length filter that drops a dated line
    // carrying no event, and the case sat GREEN because NO fixture ever fed a short dated line —
    // the property was implemented and never exercised. An empty pin is not a weak pin: it holds
    // the place of a working one, which is how `d9-01` let `28` reach a paid document.
    const out = await renderMd({ _fact_pack: [
      '## SECTION 5: News catalysts',
      '- 2026-07-02 — итог.',
      '- 2026-07-02 — Oracle signed a multi-year cloud agreement with a named counterparty.',
    ...
    const t = out.slice(out.indexOf('## Катализаторы'), out.indexOf('## Что у компании'));
    assert(/multi-year cloud agreement/.test(t), 'the real event was dropped');
    assert(!/итог\./.test(t), 'a dated line with no event in it was published as a catalyst');
  });
  ...
  await check('D8: Russian counters agree with their numbers', async () => {
    ...
    // v4.2.83: the counted noun moved from the LIVE count to the TOTAL when the metric went
    // three-state (mandate 07 §4). The grammar property is unchanged and still generated.
    const one = await renderMd({ fp_vectors: { total: 1, unverified: 0, empty: 0, caveated: 0, live: 1 } });
    const two = await renderMd({ fp_vectors: { total: 2, unverified: 1, empty: 1, caveated: 0, live: 1 } });
    const five = await renderMd({ fp_vectors: { total: 5, unverified: 0, empty: 0, caveated: 0, live: 5 } });
    ...
    assert(/собрано 1 раздел[ ,—]/.test(one), 'singular is wrong: '
      + (one.match(/собрано \d+ \S+/) || [])[0]);
    assert(/собрано 2 раздела[ ,—]/.test(two), 'the 2-4 form is wrong: '
      + (two.match(/собрано \d+ \S+/) || [])[0]);
    assert(/собрано 5 разделов[ ,—]/.test(five), 'the plural form is wrong: '
      + (five.match(/собрано \d+ \S+/) || [])[0]);
  });
  ...
  // ---- v4.2.83 (мандат 07) --------------------------------------------------------------
  await check('мандат 07 §1: the exit multiple prints ONE string in every place it appears',
    async () => {
      // The ORCL 2026-08-05 brief printed `28` in three places and `27.53` in a fourth — the
      // SAME field, `inputs.future_pe`. Two formats of one number are not a style question here:
      // the two-lens table puts them in adjacent cells, where a difference means a factor differs.
      const hits = md.match(/выходной множитель ([\d.,—]+)/g) || [];
      assert(hits.length >= 2, 'the multiplier is printed fewer times than expected: ' + hits.length);
      const vals = hits.map(h => h.replace(/выходной множитель /, ''));
      assert(new Set(vals).size === 1,
        'the exit multiple printed in more than one format: ' + JSON.stringify(vals));
      const row = md.match(/\| множитель \| ([\d.,—]+) \| ([\d.,—]+) \|/);
      if (row) assert(row[1].split('.').length === row[2].split('.').length,
        'the two-lens row prints the two multiples in different formats: ' + row[1] + ' vs ' + row[2]);
    });

  await check('мандат 07 §1: the printed arithmetic of the step table reconciles', async () => {
    // 14.27 x 28 = 399.56 was printed above a result of 392.85. The computation was right and
    // the printing was wrong, which is the version a reader catches with a calculator.
    //
    // The multiple here is deliberately NON-INTEGER. The shared fixture carries future_pe = 30,
    // which survives any rounding, so a pin driven by it cannot see this defect at all — the
    // mutation probe said so out loud (xmult-02 came back GREEN on the first draft). A fixture
    // that cannot express the failure is rule 5 again, in the fixture that was written FOR the
    // failure. 27.53 is the ORCL number, and it is the number that rounds to a lie.
    const out = await renderMd({ ivc_base: { intrinsic_value: 68.96, implied_cagr_pct: 11.56,
      fv10_per_share: 196.56, eps_terminal_dilution_adj: 7.14,
      inputs: { price: 71.71, base_per_share: 2.528, g: 0.12569, future_pe: 27.53,
                dilution_cagr: -0.00888, hurdle: 0.12, discount_rate: 0.12, terminal_g: 0.04 } } });
    const _lines = out.split('\n');
    const _epsLine = _lines.find(l => l.indexOf('прибыль десятого года') !== -1) || '';
    const _mulLine = _lines.find(l => l.indexOf('выходной множитель') !== -1
                                   && l.indexOf('|') === 0) || '';
    assert(_epsLine && _mulLine, 'the step table did not render in the expected shape');
    const _epsM = _epsLine.match(/\$([\d.]+)/);
    const _mulNums = _mulLine.match(/[\d.]+/g) || [];
    assert(_epsM && _mulNums.length >= 2, 'the step rows carry no readable numbers');
    const eps = Number(_epsM[1]), mul = Number(_mulNums[0]), fv = Number(_mulNums[1]);
    assert(Math.abs(eps * mul - fv) / fv < 0.005,
      'printed arithmetic does not reconcile: ' + eps + ' x ' + mul + ' = ' + (eps * mul).toFixed(2)
      + ', printed ' + fv);
  });

  await check('мандат 07 §3: a missing label_ru is MARKED, never silently English', async () => {
    // The v4.2.82 pin was named "with a VISIBLE fallback" and asserted only that the English
    // string appeared. It passed while the ORCL brief shipped six unmarked English drivers: a pin
    // that checks the form and not the property forbids a change, not a defect.
    const out = await renderMd({ bull_bear: { rows: [
      { side: 'BULL', label: 'OCI hypergrowth extends corridor', probability: 0.3, delta_iv_pct: 20 },
      { side: 'BEAR', label: 'Capex financing pressure', probability: 0.4, delta_iv_pct: -13 }] } });
    assert(/OCI hypergrowth extends corridor/.test(out), 'the row vanished instead of degrading');
    const line = out.split('\n').find(l => /OCI hypergrowth extends corridor/.test(l)) || '';
    assert(/по-русски не пришло/.test(line),
      'the English fallback carries no visible gap marker ON THE ROW: ' + line.trim());
  });

  await check('мандат 07 §3 control: a supplied label_ru carries NO marker', async () => {
    const out = await renderMd({ bull_bear: { rows: [
      { side: 'BULL', label: 'OCI hypergrowth', label_ru: 'Ускорение облачного бизнеса',
        probability: 0.3, delta_iv_pct: 20 }] } });
    const line = out.split('\n').find(l => /Ускорение облачного бизнеса/.test(l)) || '';
    assert(line && !/по-русски не пришло/.test(line),
      'a row that DID arrive in Russian was marked as a gap: ' + line.trim());
    assert(!/OCI hypergrowth/.test(line), 'the English label leaked beside the Russian one');
  });

  await check('мандат 07 §4: the leaf metric prints THREE states, not two', async () => {
    assert(/с оговоркой/.test(md), 'the CAVEATED state is not printed');
    assert(!/собран 1 раздел из 9/.test(md), 'the old two-state sentence is still published');
    assert(/Раздел с оговоркой НЕ пуст/.test(md),
      'a caveated section is still presented as if nothing was collected');
  });

  await check('мандат 07 §4: the incomparability warning reaches the human document', async () => {
    // The mandate asked for `states_note` VERBATIM. It cannot be: the note is English and contains
    // `[UNVERIFIED]`, which the no-jargon pin and D6 both forbid in this document. The warning is
    // therefore generated in Russian here and the verbatim string stays in the machine report.
    // Escalated to the architect; this pin guards the WARNING, not the wording of a foreign note.
    assert(/сравнивать нельзя/.test(md),
      'the reader is not warned that the percentages are not comparable across the change');
    assert(md.indexOf('UNVERIFIED') === -1, 'the English note was pasted after all');
  });

  await check('мандат 07 §4: a pre-v4.2.82 RESULT states the absence instead of inventing a split',
    async () => {
      const out = await renderMd({ _fp_vectors: { total: 14, unverified: 14, pct: 1.0,
                                                  data_questionable: true, threshold: 0.30 },
                                   fp_vectors: undefined });
      assert(/трёхсостоянийная метрика в этом прогоне отсутствует/.test(out),
        'an old RESULT was silently rendered as if it carried the three-way split');
    });

  await check('мандат 07 §4: the empty-news sentence names no section from a constant', async () => {
    // "разделы о регуляторике, конкуренции и реакции рынка не приводятся" was printed from a
    // constant over a pack whose §4 and §6 were substantial. Rule 10.
    assert(!/разделы о регуляторике, конкуренции и реакции/.test(md),
      'the hardcoded absence sentence is still published');
  });

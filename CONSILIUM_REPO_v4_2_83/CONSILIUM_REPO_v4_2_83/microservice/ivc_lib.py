...
def gps_quant(gt):
    ...
    # v4.2.82 mandate (3), ORCL 2026-08-05 — THE CASE OF RECORD FOR THIS BLOCK.
    # EDGAR returned total_debt = 0 (the LongTermDebt tag came back empty); the `gather` source
    # reported $129.541B against equity of $42.508B, i.e. D/E ~ 3.05, and the ground truth carried
    # a 100% divergence between the two. `debt_to_equity` was therefore computed as a literal 0.00,
    # scored 4/4 under "v < 0.5", and the memo wrote "zero reported leverage ... support a high
    # score" about a company with roughly $100B of debt. The run's own auditor sustained a MAJOR
    # objection against it and the score did not move.
    #
    # Three separate failures, and the fix addresses each:
    #   (1) the divergence WAS recorded — under `divergence.total_debt` — and the uncertainty
    #       detector below looked in four other places, none of them that one. A guard that reads
    #       everywhere except where the evidence lives is not a guard.
    #   (2) a leverage ratio of EXACTLY zero is not a business fact about any operating company of
    #       this size. It is the "zero instead of unknown" class landing on a balance-sheet field,
    #       the same class as MA's $21M debt tag and NFLX's zeroed series.
    #   (3) an uncertain reading was DOCKED one point (v4.2.32) but still scored. Docking treats
    #       the number as usable-but-worse. It is not usable: the honest output is a REFUSAL, which
    #       `_sub` already expresses — the sub-block leaves both the score and the maximum, and the
    #       block reports out of a smaller denominator instead of pretending to know.
    _div = (gt.get("divergence") or {})
    _dd = (_div.get("total_debt") or {}) if isinstance(_div, dict) else {}
    # v4.2.83 (mandate 07 §2) — THE FIX ABOVE NAMED THE RIGHT KEY AT THE WRONG LEVEL.
    # ORCL 2026-08-05 ran with this guard in place and the refusal still fired through
    # `_debt_zero_suspect`, not through `_debt_diverges`: the printed reason was "debt_to_equity is
    # exactly 0". The evidence was in the payload the whole time — `Gather Data` publishes it as
    # `_edgar: { flags, divergence, sources, cik, entity_name }`, so the divergence lives at
    # `_edgar.divergence.total_debt` (edgar 0 vs gather 129_541_000_000, pct 100) and NOT at the
    # top level this code read. The asymmetry was visible in the run and nobody read it that way:
    # the memo cited "GROUND_TRUTH divergence 100%" correctly, because `Prompts Growth` points the
    # model at `_edgar.divergence`, while the deterministic guard beside it saw nothing.
    #
    # So ORCL was saved by the OTHER branch, which fits this case by accident. A company with a
    # 100% source divergence and a NON-zero D/E would still have taken full marks — the class was
    # declared closed while its general form stayed open. The operator's wording, in the registry:
    # the watchman was given a second eye while the first went on looking the wrong way.
    #
    # Both levels are read, nested FIRST because that is where the producer writes it. The
    # top-level read is kept, not replaced: a payload shape is not a promise, and a guard that
    # accepts the evidence wherever the producer chose to put it is the cheap half of the fix.
    _edg = gt.get("_edgar")
    _div_nested = ((_edg.get("divergence") or {}) if isinstance(_edg, dict) else {})
    _dd_nested = (_div_nested.get("total_debt") or {}) if isinstance(_div_nested, dict) else {}
    if not _dd and isinstance(_dd_nested, dict):
        _dd = _dd_nested
    _debt_diverges = bool(_dd) and (_dd.get("pct") or 0) >= 25
    ...
    if _debt_uncertain:
        d1, md1 = None, 0        # REFUSED: not 4/4, not 0/4 — absent from the denominator
    ...
    out["detail"]["D"] = {"de": de, "dilution_cagr": dilc, "sbc_rev": sbcr, "pts": pts, "max": mx,
                          ...
                          "de_refused": bool(_debt_uncertain),
                          "de_refusal_reason": (
                              "total_debt diverges %s%% between sources (edgar %s vs gather %s)"
                              % (_dd.get("pct"), _dd.get("edgar"), _dd.get("gather"))
                              if _debt_diverges else
                              "debt_to_equity is exactly 0 — read as UNKNOWN, not as a debt-free "
                              "balance sheet" if _debt_zero_suspect else
                              "the pipeline flagged the debt reading as uncertain")
                          if _debt_uncertain else None}

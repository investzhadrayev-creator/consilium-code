...
def facts():
    ...
    mm = re.search(r"catalogue: (\d+) cases, (\d+) RED, (\d+) SKIP, (\d+) GREEN, (\d+) NOT_EXEC",
                   q.stdout + q.stderr)
    ...
    # v4.2.83 (mandate 09 §1): NOT_EXEC joins SKIP and GREEN as a reason to refuse. A case whose
    # checks never ran has proved nothing about its pin, so a catalogue containing one cannot be
    # quoted as evidence — and the summary line is REQUIRED to carry the counter, because a regex
    # that silently tolerates its absence would go on packaging the day the counter disappears.
    f["mutations_notexec"] = int(mm.group(5)) if mm else None
    f["mutations_clean"] = (mm is not None and q.returncode == 0
                            and f["mutations_skip"] == 0 and f["mutations_green"] == 0
                            and f["mutations_notexec"] == 0)
    return f
...
def main():
    ...
    if not f["mutations_clean"]:
        print("\n!! mutation catalogue is not clean (SKIP = refusal of the check, GREEN = empty "
              "pin, NOT_EXEC = never probed) — the package must not quote it:",
    ...
    print("  каталог мутаций: %d кейсов, %d RED, %d SKIP, %d GREEN, %d NOT_EXEC"
          % (f["mutations"], f["mutations_red"], f["mutations_skip"], f["mutations_green"],
             f["mutations_notexec"]))
